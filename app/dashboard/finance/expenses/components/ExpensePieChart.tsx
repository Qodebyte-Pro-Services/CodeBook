...
const ExpensePieChart: React.FC<ExpenseSummaryProps> = ({
  totalAmount,
  month,
  categories,
}) => {
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-md">
      <div className="flex justify-between items-center mb-6">
        <h4 className="text-lg font-semibold text-gray-800">Total Amount Spent</h4>
      ...
      <div className="relative w-48 h-48 mx-auto flex items-center justify-center">
        ...
        >
          <p className="text-lg font-semibold text-gray-800">{`N${(totalAmount / 1000).toFixed(0)}K`}</p>
        </CircularProgressbarWithChildren>
        ...
        <svg className="absolute top-0 left-0 w-full h-full">
          ...
          {categories.map((category, index) => {
            const startAngle =
              (categories.slice(0, index).reduce((sum, cat) => sum + cat.value, 0) / total) *
              360;
